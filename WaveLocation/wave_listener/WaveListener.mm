...
- (void)computeWave {
    ...
    if (_isListening == YES && fftBufferManager->HasNewAudioData()) {
...
- (void)helperResultWithTimeSlice:(int)length {
    ...
    if (queue_item_at_index(q17, 0) > 0.0 &&
        ...
        if (counter == 0) {
            ...
            if (self.listenedActionDelegate != nil && [self.listenedActionDelegate respondsToSelector:@selector(listenedAction:)]) {
                
                [self.listenedActionDelegate listenedAction:string];
